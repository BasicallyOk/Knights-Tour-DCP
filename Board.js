export class Board {
    ...
    merge (partitions) {
        /*
        Merge board partitions into one whole board
        */
       return null;
    }
    ...
    */
    ...
    async function workFn(array, currLoc) {
        ...
        function isValid (location) {
            return location[0] <= array.length && location[1] <= array[0].length
        }

        function notVisited(location) {
            return array[location[0]][location[1]] !== -1
        }
        ...
        function next() {
            ...
            for (let i = 0; i < moves.length; i++) {
                ...
                let degree
                if (!isValid(moves[index]) || !notVisited(moves[index])) {
                    continue
                } else {
